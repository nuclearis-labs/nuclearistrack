...
const roleBasedAclABI = require('../../build/contracts/NuclearPoE.json').abi;
...
export async function confirm(req: Request, res: Response) {
  ...
  try {
    ...
    const rolesContract = new Contract({
      privateKey,
      abi: roleBasedAclABI,
      contractAddress: process.env.ROLEADDRESS
    });
    console.log(newAddress);
    console.log(user.roles);
    console.log(process.env.ROLEADDRESS);

    const txHash = await rolesContract.sendDataToContract({
      fromAddress: address,
