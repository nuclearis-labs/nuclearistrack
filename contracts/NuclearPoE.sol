...
contract NuclearPoE is Ownable {
    ...
    modifier onlyClient(address _clientAddress) {
        ...
        require(
            user[msg.sender].userType == Type.Client,
            'Sender is not client type'
        );
        _;
    }
    ...
    function createProject(
        ...
        require(
            user[_clientAddress].userType == Type.Client,
            'User has to be Client Type'
        );
    ...
    function createUser(address _userAddress, Type _userType, bytes32 _userName)
        external
        onlyOwner()
    {
        require(
            user[_userAddress].status == State.Null,
    ...
    function changeUserStatus(address _userAddress)
        external
        onlyOwner()
        onlyExistingUser(_userAddress)
    {
        if (user[_userAddress].status == State.Created)
            user[_userAddress].status = State.Closed;
    ...
    function createProcess(address _supplierAddress, bytes32 _processName)
        external
        onlyOwner()
        onlyExistingUser(_supplierAddress)
    {
        require(
            user[_supplierAddress].userType == Type.Supplier,
            'User has to be Supplier Type'
        );
    ...
    function getSupplierProjects(address _address)
        external
        view
        returns (address[] memory)
    {
    ...
    function getClientProjects(address _address)
        external
        view
    ...
    function getProjectDetails(uint256 _id)
        external
        view
        returns (State, address, bytes32, bytes32, bytes32, address[] memory)
    {
